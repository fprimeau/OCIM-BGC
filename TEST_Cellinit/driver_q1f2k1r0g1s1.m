...
fprintf(['\n' 'NOTE: test cell model initial values \n' ...
	...
	'sigma fixed at zero, \n'...
	'beta fixed at 0.5 \n'...
	'alpha fixed at 3.78e-5'
	'Q10Photo on initial = 1 \n'...
	'fStorage initialized at 1 \n'...
	'rcutoff fixed at 2.25 \n'...
	'alphaS initialized at 0.225 2.25 \n'...
	'dynamicP = off \n'])
...
if isempty(gcp('nocreate'))
	poolobj = parpool(3); % if running in parallel; comment out to run serial
...
VerName = 'a0q1f2k1r0g1s1_'; 		% optional version name. leave as an empty character array
...
% if (par.optim == on) | (par.LoadOpt == on)
	...
	if exist('xhat') & isfield(xhat,'alphaS')
		...
		%v2
		%par.BIO.alphaS = 1.0;   % get more N limitation if alphaS is larger. also min celll size is larger
	end
