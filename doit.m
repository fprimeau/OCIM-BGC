...
if par.C13model == on 
  ...
  DOC  = M3d+nan ;  DOC(iwet)  = C(2*nwet+1:3*nwet) ;
  PIC  = M3d+nan ;  PIC(iwet)  = C(3*nwet+1:4*nwet) ;
  DOCl = M3d+nan ;  DOCl(iwet) = C(4*nwet+1:5*nwet) ;
  DOCr = M3d+nan ;  DOCr(iwet) = C(5*nwet+1:6*nwet) ;

  par.DIC13 = DIC(iwet) ;
  par.POC13 = POC(iwet) ;
  par.DOC13 = DOC(iwet) ;
  par.DOC13l = DOCl(iwet) ;
  par.DOC13r = DOCr(iwet) ;
  % DIC = DIC + par.dicant  ;

  data.DIC13  = DIC  ;  data.POC13  = POC ;
  data.DOC13  = DOC  ;  data.PIC13  = PIC ;
  data.DOC13r = DOCr ;  data.DOC13l = DOCl ;
end
