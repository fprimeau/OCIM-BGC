...
% unpack some useful stuff
    ...
    G_alk   = vout.G_alk ;
    JgDIC   = vout.JgDIC ;
    co2surf = vout.co2surf; % unit check. umol/kg ?
    clear vout;
    ...
    % WARNING: we should resolve the CO2 system at  all the layers  where we have biological production. For now we approximate the CO2 at all layers
    % using co2surf. 
    ...
    elseif (par.optim & nargout > 3);
        ...
        [~,~,Gxx] = uptake_C(par);
        par.Gxx   = Gxx;
        DICx  = C13x(0*nwet+1:1*nwet, : );
        POCx  = C13x(1*nwet+1:2*nwet, : );
        DOCx  = C13x(2*nwet+1:3*nwet, : );
        PICx  = C13x(3*nwet+1:4*nwet, : );
        %ALKx  = Cx(4*nwet+1:5*nwet, : );
        DOClx = C13x(4*nwet+1:5*nwet, : );
        DOCrx = C13x(5*nwet+1:6*nwet, : );
        % ------------------------------------------------------
