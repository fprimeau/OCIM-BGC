...
elseif Gtest == off
    ...
    elseif (par.Cmodel == on & par.Omodel == on & par.Simodel == off)
        base_name = strcat(VER,'_C_isotope');
...
if par.Cmodel == on 
    ...
    %
   % GC13  = [data.DIC(iwet); data.POC(iwet); data.DOC(iwet); data.PIC(iwet); ...
   %         data.DIC(iwet); data.POC(iwet);];
   % GC13 = (10^(-2) )*real(GC13) + 1e-8 *randn(6*nwet,1) ;
    
   % GC14  = [data.DIC(iwet); data.POC(iwet); data.DOC(iwet); data.PIC(iwet); ...
   %        data.DIC(iwet); data.POC(iwet);];
   % GC14 = (10^(-12))*real(GC14) + 1e-18*randn(6*nwet,1) ;
