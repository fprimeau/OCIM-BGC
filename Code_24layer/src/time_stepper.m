function [Xout,Tout] = time_stepper(par,t0,t1,Xin,Ctype)
  ...
  % Ctype = {14};
  % set up A,B,FA for time-stepping C12,C13 and C14
  ...
  for i = 2:nstep
    ...
    fprintf('...Time: %4.2f, AtmCO2: %4.2f,R13a: %7.6g, R14a: %7.6g, DI%s: %7.6g\n',...
                t,max(par.pco2atm(:)), max(par.c13.R13a),max(par.c14.R14a(:)),...
                vn,eval(sprintf('mean(%s(1:nwet))',vn)));

    % time stepping C12, C13 and C14
    for m = 1:length(Ctype)
      vn = Ctype{m};
    ...
    if ~par.saveall
      if mod(i,Nstep) == 0
       % outname = sprintf('%smodel_%s_%i.mat',par.output_dir,strjoin(Ctype,'_'),i/Nstep);
         outname = sprintf('%smodel_%s_%i.mat',par.output_dir,strjoin(Ctype,'_'),273);
        save(outname,'Xout','-v7.3');
        sprintf('saving file')
      end
    end
