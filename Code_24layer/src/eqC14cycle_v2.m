function [par, C14, C14x, C14xx] = eqC14cycle_v2(x, par);
...
% output: Cxx hessian matrix of C w.r.t.  model parameters x
    ...
    %
    options.iprint = 1   ; 
    options.atol = 1e-25 ; %1e-13에 1e-12(Trillion 곱하기) ;
    options.rtol = 1e-13 ;
...
% unpack some useful stuff
    ...
    disp(sprintf('photosynthesis fractionation tuned by frpho=%4.2f and fc14=%4.2f',par.frpho,fc14));
    ...
    eq1 = TRdiv*DIC14 ...                         % advective-diffusive transport
          + d0(par.Cnpp(iwet).*alpha_dic2poc)*R14o ...   % removal of dic14 organic c14 production
    ...
    eq5 = (TRdiv+kappa_l*I)*DOC14l - d0((par.Cnpp(iwet)-G*C2P).*alpha_dic2poc)*R14o + lambda14*DOC14l;  % DOCl 
    ...
    % colum 1 dFdDIC14
    Jc{1,1} = TRdiv + d0(par.Cnpp(iwet).*alpha_dic2poc)*dR14o ...
    ...
    Jc{5,1} = - d0((par.Cnpp(iwet)-G*C2P).*alpha_dic2poc)*dR14o ; 
    Jc{6,1} = 0*I ;
    % colum 2 dFdPOC14
    Jc{1,2} = -kappa_p*I ;
    ...
    elseif (par.optim & nargout > 3)
        ...
        % kru 
        if (par.opt_kru == on)
            tmp = kru*[UM*DOCr; Z; Z; Z; Z; -UM*DOCr];
        ...
        % krd 
        if (par.opt_krd == on)
            tmp = krd*[DM*DOCr; Z; Z; Z; Z; -DM*DOCr];
    ...
    elseif (par.optim & nargout > 3);
        ...
        [~,~,Gxx] = uptake_C(par);
        par.Gxx   = Gxx;
        DICx  = C14x(0*nwet+1:1*nwet, : );
        POCx  = C14x(1*nwet+1:2*nwet, : );
        DOCx  = C14x(2*nwet+1:3*nwet, : );
        PICx  = C14x(3*nwet+1:4*nwet, : );
        %ALKx  = Cx(4*nwet+1:5*nwet, : );
        DOClx = C14x(4*nwet+1:5*nwet, : );
        DOCrx = C14x(5*nwet+1:6*nwet, : );
        % ------------------------------------------------------
        ...
        if (par.opt_sigC & par.opt_bC_T == on)
            kk = kk + 1;
            tmp = [Z; -PFD_bm*POCx(:,pindx.lsigC); Z;  Z; Z; Z];
        ...
        if (par.opt_sigC & par.opt_bC == on)
            kk = kk + 1;
            tmp = bC*[Z; -PFD_bb*POCx(:,pindx.lsigC); Z;  Z; Z; Z];
        ...
        if (par.opt_sigC & par.opt_d == on)
            kk = kk + 1;
            tmp = d*[Z; Z; Z; -PFD_d*PICx(:,pindx.lsigC); Z; Z];
