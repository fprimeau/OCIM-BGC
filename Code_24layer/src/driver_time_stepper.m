%
%
% disp(sprintf('loading steady state from %s',));
...
output_dir = sprintf('MSK%2d/log_Results/',GridVer); 
VER = strcat(output_dir);
fopt = 'Reoptimized_solution';
...
% pack parameters into an array, assign them corresponding indices.
% par = PackPar(par) ;
% ----> 위에서 이미 updated. PackPar는 필요 없는 듯.
...
% solve the steady state of C, C13, C14
...
% drive the time stepping method for C, C13, C14
