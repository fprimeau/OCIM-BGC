...
%RunVer = 'futureproj/Tup5Cv2_Cell_CTL_He_PCCell';
RunVer = 'futureproj/Tup5Cv2_GM15_CTL_He_PC'
%'optC_Tz_CTL_He_PC_DOC0.25_DOP0'

outputDir = '/DFS-L/DATA/primeau/meganrs/OCIM_BGC_OUTPUT/C2P_paper_optC/';
figDir = strcat(outputDir,'futureproj/FIGS_Tup5C_GM15/v2_');
outPath = figDir;
...
if isfield(xhat,'cc')
	...
	C2P = M3d + nan  ;
	C2Popt = M3d + nan  ;
	C2P(iwet)  = 1./(cc*par.po4obs(iwet) + dd) ;
	...
	h(2) = fill([lat(ind1),fliplr(lat(ind1))],[(C2P_latavg(ind1)-C2P_latstd(ind1))', fliplr((C2P_latavg(ind1)+C2P_latstd(ind1))')],'r','LineStyle','none'); alpha(0.1); hold on

	h(3) = plot(lat,C2Popt_latavg,'-ko'); hold on
	%plot(lat,C2P_latmedian1,'-.b','linewidth',2);
	h(4) = plot(lat,C2P_latavg,'-ro');
	%plot(lat,C2P_latmedian2,'-.m','linewidth',2);
	legend(h([3 4]),'optimal','T+5C');
	xlabel('Latitude')
	ylabel(['C:P [molC:molP] = 1/(' num2str(cc) '*[DIP] + ' num2str(dd) ')']);
	title('Zonal Average Biological C:P')
	axis tight; grid on
	%ylim([0 300])
	figTitle = 'C2P_lat_avg_Tup5Cvopt';
	...
	axis tight; grid on
	ylim([0 300])
	figTitle = 'C2P_lat_avg_basin_Tup5C';
	print(gcf,[outPath 'FIG_' figTitle '.png'],'-dpng')
	clear h;
...
axis xy;
title('Future - preindustrial difference in Total Organic Carbon Export')
xlabel('Longitude');
ylabel('Latitude');
