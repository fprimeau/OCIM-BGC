...
%}
        ...
        if (par.opt_kP_T == on)
			%new:
			if (par.Cellmodel)
	   			tmp = [-(I+(1-sigma)*RR)*(d0(Gx(:,pindx.kP_T))*C2P + G*C2Px(:,pindx.kP_T)); ...
		...
		% I want to change this section to only call C2Px (instead of par.BIO).
		% It should be an easy swap. C2Px is already w.r.t. the transformed parameters in x
		% But this is working and I dont want to introduce any new bugs until I
		% fix the current problems with the hessian
		%  Q10Photo
		...
		if (par.opt_fRibE == on)
			...
			dfRibE_tfRibE = 0.5*sech(tfRibE)^2; 	% C2P_fRibE*dfRibE_tfRibE =C2Px(:,par.pindx.tfRibE)
		...
		if (par.opt_PLip_PCutoff == on)
        	C2P_PCutoff = par.CellOut.dC2P_dPCutoff(iwet); % C2P_PCutoff*dPCutoff_lPCutoff = C2Px(:,par.pindx.lPLip_PCutoff);
		...
		if (par.opt_PLip_scale == on)
        	C2P_PLipscale = par.CellOut.dC2P_dPLip_scale(iwet); % C2P_PLipscale*dPLipscale_lPLipscale =C2Px(:,par.pindx.lPLip_scale);
		...
		if (par.opt_PStor_rCutoff == on)
        	C2P_rCutoff = par.CellOut.dC2P_drCutoff(iwet); % C2P_rCutoff*drCutoff_lrCutoff =C2Px(:,par.pindx.lPStor_rCutoff);
		...
		if (par.opt_PStor_scale == on)
        	C2P_PStorscale = par.CellOut.dC2P_dPStor_scale(iwet); % =C2P_PStorscale*dPStorscale_lPStorscale = C2Px(:,par.pindx.lPStor_scale);
		...
		if (par.opt_alphaS == on)
        	C2P_alphaS = par.CellOut.dC2P_dalphaS(iwet); %C2P_alphaS*dalphaS_lalphaS =C2Px(:,par.pindx.lalphaS);
...
%}
        ...
        for jj = 1:npx
            for jk = jj:npx
                kk = kk + 1;
                if (par.opt_sigma == on)
                    % sigma sigma %% HAS NOT BEEN UPDATED FOR CELL MODEL
                    if (jj == jk & jj == pindx.lsigma)
						if (par.Cellmodel)
							%% INCORRECT. THIS HAS NOT BEEN UPDATED FOR CELL MODEL
							d2GxC2P = (d0(Gxx(:,kk))*C2P +G*C2Ppxx(:,kk) +d0(Gx(:,jj))*C2Px(:,jk)+d0(Gx(:,jk))*C2Px(:,jj));
							tmp = sigma*[RR*G*C2P + 2*d0(RR*Gx(:,jj))*C2P; ...
                                     -G*C2P - 2*d0(Gx(:,jj))*C2P;...
                                      G*C2P + 2*d0(Gx(:,jj))*C2P;...
                                     -RR*G*C2P - 2*d0(RR*Gx(:,jj))*C2P; ...
                                     2*RR*G*C2P + 2*2*d0(RR*Gx(:,jj))*C2P] + ...
                              [-(I+(1-sigma)*RR)*d2GxC2P; ...
                               (1-sigma)*d2GxC2P; ...
                               sigma*d2GxC2P; ...
                               (1-sigma)*RR*d2GxC2P; ...
                               -2*(1-sigma)*RR*d2GxC2P + ...
                               N2C*d2GxC2P];
                    ...
                    elseif (jj ~= pindx.lsigma & jk ~= pindx.lsigma)
						...
						% (sigma paired with another P model parameter)
						if (par.Cellmodel)
					...
					% if sigma is turned off, all P model parameters have same hessian form:
					% note: C2Ppxx is the second deriv of Cell model C:P wrt P model parameters (2nd deriv for cell model params stored in C2Pxx)
					if (par.Cellmodel)
						% debugging: define long G*C2Pxx, check this derivative of G*C2P wrt P model parameters
					    tmp = [-(I+(1-sigma)*RR)*(d0(Gxx(:,kk))*C2P +G*C2Ppxx(:,kk) +d0(Gx(:,jj))*C2Px(:,jk)+d0(Gx(:,jk))*C2Px(:,jj)); ...
							(1-sigma)*(d0(Gxx(:,kk))*C2P +G*C2Ppxx(:,kk) +d0(Gx(:,jj))*C2Px(:,jk)+d0(Gx(:,jk))*C2Px(:,jj)); ...
							sigma*(d0(Gxx(:,kk))*C2P +G*C2Ppxx(:,kk) +d0(Gx(:,jj))*C2Px(:,jk)+d0(Gx(:,jk))*C2Px(:,jj)); ...
							(1-sigma)*RR*(d0(Gxx(:,kk))*C2P +G*C2Ppxx(:,kk) +d0(Gx(:,jj))*C2Px(:,jk)+d0(Gx(:,jk))*C2Px(:,jj)); ...
        ...
        % sigma R_Si  *incorrect for cell model
        ...
        % sigma rR  %% incorrect for cell model
...
%kP_T
        ...
        if (par.opt_kP_T & par.opt_R_Si)
            kk = kk + 1;
			if (par.Cellmodel)
				tmp = [-(1-sigma)*RR_Si*(d0(Gx(:,pindx.kP_T))*C2P + G*C2Px(:,pindx.kP_T)); ...
                   	Z ; ...
                   	Z ; ...
                   	(1-sigma)*RR_Si*(d0(Gx(:,pindx.kP_T))*C2P + G*C2Px(:,pindx.kP_T)); ...
                   	-2*(1-sigma)*RR_Si*(d0(Gx(:,pindx.kP_T))*C2P + G*C2Px(:,pindx.kP_T))];
        ...
        if (par.opt_kP_T & par.opt_rR)
			kk = kk + 1;
			if (par.Cellmodel)
				tmp = [-(1-sigma)*RR_rR*(d0(Gx(:,pindx.kP_T))*C2P + G*C2Px(:,pindx.kP_T)); ...
                   Z ; ...
                   Z ; ...
                   (1-sigma)*RR_rR*(d0(Gx(:,pindx.kP_T))*C2P + G*C2Px(:,pindx.kP_T)); ...
                   -2*(1-sigma)*RR_rR*(d0(Gx(:,pindx.kP_T))*C2P + G*C2Px(:,pindx.kP_T))];
	...
	%kdP
        ...
        if (par.opt_kdP & par.opt_R_Si)
            kk  = kk + 1;
			if (par.Cellmodel)
				tmp = [-(1-sigma)*RR_Si*(d0(Gx(:,pindx.lkdP))*C2P + G*C2Px(:,pindx.lkdP)); ...
                   Z ; ...
                   Z ; ...
                   (1-sigma)*RR_Si*(d0(Gx(:,pindx.lkdP))*C2P + G*C2Px(:,pindx.lkdP)); ...
                   -2*(1-sigma)*RR_Si*(d0(Gx(:,pindx.lkdP))*C2P + G*C2Px(:,pindx.lkdP))];
        ...
        if (par.opt_kdP & par.opt_rR)
            kk  = kk + 1;
			if (par.Cellmodel)
				tmp = [-(1-sigma)*RR_rR*(d0(Gx(:,pindx.lkdP))*C2P + G*C2Px(:,pindx.lkdP)); ...
                   Z ; ...
                   Z ; ...
                   (1-sigma)*RR_rR*(d0(Gx(:,pindx.lkdP))*C2P + G*C2Px(:,pindx.lkdP)); ...
                   -2*(1-sigma)*RR_rR*(d0(Gx(:,pindx.lkdP))*C2P + G*C2Px(:,pindx.lkdP))];
...
%bP_T
        ...
        if (par.opt_bP_T & par.opt_R_Si)
			...
			if (par.Cellmodel)
				...
				tmp = [-(1-sigma)*RR_Si*(d0(Gx(:,pindx.bP_T))*C2P + G*C2Px(:,pindx.bP_T)); ...
                   Z ; ...
                   Z ; ...
                   (1-sigma)*RR_Si*(d0(Gx(:,pindx.bP_T))*C2P + G*C2Px(:,pindx.bP_T)) ; ...
                   -2*(1-sigma)*RR_Si*(d0(Gx(:,pindx.bP_T))*C2P + G*C2Px(:,pindx.bP_T))] ;
        ...
        if (par.opt_bP_T & par.opt_rR)
            kk = kk + 1;
			if (par.Cellmodel)
				tmp = [-(1-sigma)*RR_rR*(d0(Gx(:,pindx.bP_T))*C2P + G*C2Px(:,pindx.bP_T)); ...
                   	Z ; ...
                   	Z ; ...
                   	(1-sigma)*RR_rR*(d0(Gx(:,pindx.bP_T))*C2P + G*C2Px(:,pindx.bP_T)); ...
                   	-2*(1-sigma)*RR_rR*(d0(Gx(:,pindx.bP_T))*C2P + G*C2Px(:,pindx.bP_T))];
...
%bP
        ...
        if (par.opt_bP & par.opt_R_Si)
            kk = kk + 1;
			if (par.Cellmodel)
				tmp = [-(1-sigma)*RR_Si*(d0(Gx(:,pindx.lbP))*C2P + G*C2Px(:,pindx.lbP)); ...
                   Z ; ...
                   Z ; ...
                   (1-sigma)*RR_Si*(d0(Gx(:,pindx.lbP))*C2P + G*C2Px(:,pindx.lbP)); ...
                   -2*(1-sigma)*RR_Si*(d0(Gx(:,pindx.lbP))*C2P + G*C2Px(:,pindx.lbP))];
        ...
        if (par.opt_bP & par.opt_rR)
            kk = kk + 1;
			if (par.Cellmodel)
				tmp = [-(1-sigma)*RR_rR*(d0(Gx(:,pindx.lbP))*C2P + G*C2Px(:,pindx.lbP)); ...
                   Z ; ...
                   Z ; ...
                   (1-sigma)*RR_rR*(d0(Gx(:,pindx.lbP))*C2P + G*C2Px(:,pindx.lbP)); ...
                   -2*(1-sigma)*RR_rR*(d0(Gx(:,pindx.lbP))*C2P + G*C2Px(:,pindx.lbP))];
...
% alpha
        ...
        if (par.opt_alpha & par.opt_R_Si)
            kk  = kk + 1;
			if (par.Cellmodel)
				tmp = [-(1-sigma)*RR_Si*(d0(Gx(:,pindx.lalpha))*C2P + G*C2Px(:,pindx.lalpha)); ...
                   Z ; ...
                   Z ; ...
                   (1-sigma)*RR_Si*(d0(Gx(:,pindx.lalpha))*C2P + G*C2Px(:,pindx.lalpha)); ...
                   -2*(1-sigma)*RR_Si*(d0(Gx(:,pindx.lalpha))*C2P + G*C2Px(:,pindx.lalpha))];
        ...
        if (par.opt_alpha & par.opt_rR)
            kk  = kk + 1;
			if (par.Cellmodel)
				tmp = [-(1-sigma)*RR_rR*(d0(Gx(:,pindx.lalpha))*C2P + G*C2Px(:,pindx.lalpha)); ...
                   Z ; ...
                   Z ; ...
                   (1-sigma)*RR_rR*(d0(Gx(:,pindx.lalpha))*C2P + G*C2Px(:,pindx.lalpha)); ...
                   -2*(1-sigma)*RR_rR*(d0(Gx(:,pindx.lalpha))*C2P + G*C2Px(:,pindx.lalpha))];
...
% beta
        ...
        if (par.opt_beta & par.opt_R_Si)
            kk = kk + 1;
			if (par.Cellmodel)
				tmp = [-(1-sigma)*RR_Si*(d0(Gx(:,pindx.lbeta))*C2P + G*C2Px(:,pindx.lbeta)); ...
                   Z ; ...
                   Z ; ...
                   (1-sigma)*RR_Si*(d0(Gx(:,pindx.lbeta))*C2P + G*C2Px(:,pindx.lbeta)); ...
                   -2*(1-sigma)*RR_Si*(d0(Gx(:,pindx.lbeta))*C2P + G*C2Px(:,pindx.lbeta))];
        ...
        if (par.opt_beta & par.opt_rR)
            kk = kk + 1;
			if (par.Cellmodel)
				tmp = [-(1-sigma)*RR_rR*(d0(Gx(:,pindx.lbeta))*C2P + G*C2Px(:,pindx.lbeta)); ...
                   Z ; ...
                   Z ; ...
                   (1-sigma)*RR_rR*(d0(Gx(:,pindx.lbeta))*C2P + G*C2Px(:,pindx.lbeta)); ...
                   -2*(1-sigma)*RR_rR*(d0(Gx(:,pindx.lbeta))*C2P + G*C2Px(:,pindx.lbeta))];
