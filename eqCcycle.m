...
% unpack some useful stuff
    ...
    elseif (par.optim & nargout > 2)
		...
		if (par.opt_fStorage == on)
			...
			tmp = par.BIO.fStorage*[-(I+(1-sigma)*RR)*(G*C2P_fStor); ...
                      (1-sigma)*G*C2P_fStor; ...
                      sigma*G*C2P_fStor; ...
		...
		if (par.opt_PLip_PCutoff == on)
        	C2P_PCutoff = par.CellOut.dC2P_dPCutoff(iwet);
			tmp = par.BIO.PLip_PCutoff*[-(I+(1-sigma)*RR)*(G*C2P_PCutoff); ...
		...
		if (par.opt_PLip_scale == on)
        	C2P_PLipscale = par.CellOut.dC2P_dPLip_scale(iwet);
			tmp = par.BIO.PLip_scale*[-(I+(1-sigma)*RR)*(G*C2P_PLipscale); ...
		...
		if (par.opt_PStor_rCutoff == on)
        	C2P_rCutoff = par.CellOut.dC2P_drCutoff(iwet);
			tmp = par.BIO.PStor_rCutoff*[-(I+(1-sigma)*RR)*(G*C2P_rCutoff); ...
		...
		if (par.opt_PStor_scale == on)
        	C2P_PStorscale = par.CellOut.dC2P_dPStor_scale(iwet);
			tmp = par.BIO.PStor_scale*[-(I+(1-sigma)*RR)*(G*C2P_PStorscale); ...
		...
		if (par.opt_alphaS == on)
        	C2P_alphaS = par.CellOut.dC2P_dalphaS(iwet);
			tmp = par.BIO.alphaS*[-(I+(1-sigma)*RR)*(G*C2P_alphaS); ...
 ...
 % sigma + Cell model params
		...
		if (par.opt_sigma & par.opt_PLip_PCutoff)
        	kk = kk + 1;
			tmp = par.BIO.PLip_PCutoff*[RR*sigma*G*C2P_PCutoff; ...
                      -sigma*G*C2P_PCutoff; ...
                      sigma*G*C2P_PCutoff; ...
                      -RR*sigma*G*C2P_PCutoff; ...
                      2*RR*sigma*G*C2P_PCutoff] + ...
                  par.BIO.PLip_PCutoff*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lsigma))*C2P_PCutoff; ...
		...
		if (par.opt_sigma & par.opt_PLip_scale)
        	kk = kk + 1;
			dC2Ptmp = C2P_PLipscale;
			tmp = par.BIO.PLip_scale*[RR*sigma*G*dC2Ptmp; ...
                      -sigma*G*dC2Ptmp; ...
                      sigma*G*dC2Ptmp; ...
                      -RR*sigma*G*dC2Ptmp; ...
                      2*RR*sigma*G*dC2Ptmp] + ...
                  par.BIO.PLip_scale*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lsigma))*dC2Ptmp; ...
		...
		if (par.opt_sigma & par.opt_PStor_rCutoff)
        	kk = kk + 1;
			tmp = par.BIO.PStor_rCutoff*[RR*sigma*G*C2P_rCutoff; ...
                      -sigma*G*C2P_rCutoff; ...
                      sigma*G*C2P_rCutoff; ...
                      -RR*sigma*G*C2P_rCutoff; ...
                      2*RR*sigma*G*C2P_rCutoff] + ...
                  par.BIO.PStor_rCutoff*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lsigma))*C2P_rCutoff; ...
                      (1-sigma)*d0(Gx(:,pindx.lsigma))*C2P_rCutoff; ...
                      sigma*d0(Gx(:,pindx.lsigma))*C2P_rCutoff; ...
                      d0((1-sigma)*RR*Gx(:,pindx.lsigma))*C2P_rCutoff; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lsigma))*C2P_rCutoff + ...
                      d0(N2C*Gx(:,pindx.lsigma))*C2P_rCutoff];
		...
		if (par.opt_sigma & par.opt_PStor_scale)
        	kk = kk + 1;
			dC2Ptmp = C2P_PStorscale;
			tmp = par.BIO.PStor_scale*[RR*sigma*G*dC2Ptmp; ...
                      -sigma*G*dC2Ptmp; ...
                      sigma*G*dC2Ptmp; ...
                      -RR*sigma*G*dC2Ptmp; ...
                      2*RR*sigma*G*dC2Ptmp] + ...
                  par.BIO.PStor_scale*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lsigma))*dC2Ptmp; ...
                      (1-sigma)*d0(Gx(:,pindx.lsigma))*dC2Ptmp; ...
                      sigma*d0(Gx(:,pindx.lsigma))*dC2Ptmp; ...
                      d0((1-sigma)*RR*Gx(:,pindx.lsigma))*dC2Ptmp; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lsigma))*dC2Ptmp + ...
		...
		if (par.opt_sigma & par.opt_alphaS)
        	kk = kk + 1;
			dC2Ptmp = C2P_alphaS;
			tmp = par.BIO.alphaS*[RR*sigma*G*dC2Ptmp; ...
                      -sigma*G*dC2Ptmp; ...
                      sigma*G*dC2Ptmp; ...
                      -RR*sigma*G*dC2Ptmp; ...
                      2*RR*sigma*G*dC2Ptmp] + ...
                  par.BIO.alphaS*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lsigma))*dC2Ptmp; ...
                      (1-sigma)*d0(Gx(:,pindx.lsigma))*dC2Ptmp; ...
                      sigma*d0(Gx(:,pindx.lsigma))*dC2Ptmp; ...
                      d0((1-sigma)*RR*Gx(:,pindx.lsigma))*dC2Ptmp; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lsigma))*dC2Ptmp + ...
...
%kP_T
	...
	% kP_T + cell model
		...
		if (par.opt_kP_T & par.opt_PLip_PCutoff)
			kk = kk + 1;
            tmp = par.BIO.PLip_PCutoff*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.kP_T))*C2P_PCutoff; ...
                      (1-sigma)*d0(Gx(:,pindx.kP_T))*C2P_PCutoff; ...
                      sigma*d0(Gx(:,pindx.kP_T))*C2P_PCutoff; ...
                      d0((1-sigma)*RR*Gx(:,pindx.kP_T))*C2P_PCutoff; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.kP_T))*C2P_PCutoff + ...
                      d0(N2C*Gx(:,pindx.kP_T))*C2P_PCutoff];
		...
		if (par.opt_kP_T & par.opt_PLip_scale)
			kk = kk + 1;
			dC2Ptmp = C2P_PLipscale;
            tmp = par.BIO.PLip_scale*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.kP_T))*dC2Ptmp; ...
		...
		if (par.opt_kP_T & par.opt_PStor_rCutoff)
			kk = kk + 1;
            tmp = par.BIO.PStor_rCutoff*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.kP_T))*C2P_rCutoff; ...
                      (1-sigma)*d0(Gx(:,pindx.kP_T))*C2P_rCutoff; ...
                      sigma*d0(Gx(:,pindx.kP_T))*C2P_rCutoff; ...
                      d0((1-sigma)*RR*Gx(:,pindx.kP_T))*C2P_rCutoff; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.kP_T))*C2P_rCutoff + ...
                      d0(N2C*Gx(:,pindx.kP_T))*C2P_rCutoff];
		...
		if (par.opt_kP_T & par.opt_PStor_scale)
			kk = kk + 1;
			dC2Ptmp = C2P_PStorscale;
            tmp = par.BIO.PStor_scale*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.kP_T))*dC2Ptmp; ...
                      (1-sigma)*d0(Gx(:,pindx.kP_T))*dC2Ptmp; ...
                      sigma*d0(Gx(:,pindx.kP_T))*dC2Ptmp; ...
                      d0((1-sigma)*RR*Gx(:,pindx.kP_T))*dC2Ptmp; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.kP_T))*dC2Ptmp + ...
		...
		if (par.opt_kP_T & par.opt_alphaS)
			kk = kk + 1;
			dC2Ptmp = C2P_alphaS;
            tmp = par.BIO.alphaS*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.kP_T))*dC2Ptmp; ...
                      (1-sigma)*d0(Gx(:,pindx.kP_T))*dC2Ptmp; ...
                      sigma*d0(Gx(:,pindx.kP_T))*dC2Ptmp; ...
                      d0((1-sigma)*RR*Gx(:,pindx.kP_T))*dC2Ptmp; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.kP_T))*dC2Ptmp + ...
	...
	% kdP + cell model
		...
		if (par.opt_kdP & par.opt_PLip_PCutoff)
			kk = kk + 1;
			tmp = par.BIO.PLip_PCutoff*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lkdP))*C2P_PCutoff; ...
                      (1-sigma)*d0(Gx(:,pindx.lkdP))*C2P_PCutoff; ...
                      sigma*d0(Gx(:,pindx.lkdP))*C2P_PCutoff; ...
                      d0((1-sigma)*RR*Gx(:,pindx.lkdP))*C2P_PCutoff; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lkdP))*C2P_PCutoff + ...
                      d0(N2C*Gx(:,pindx.lkdP))*C2P_PCutoff];
		...
		if (par.opt_kdP & par.opt_PLip_scale)
			kk = kk + 1;
			dC2Ptmp = C2P_PLipscale;
			tmp = par.BIO.PLip_scale*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lkdP))*dC2Ptmp; ...
		...
		if (par.opt_kdP & par.opt_PStor_rCutoff)
			kk = kk + 1;
			tmp = par.BIO.PStor_rCutoff*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lkdP))*C2P_rCutoff; ...
                      (1-sigma)*d0(Gx(:,pindx.lkdP))*C2P_rCutoff; ...
                      sigma*d0(Gx(:,pindx.lkdP))*C2P_rCutoff; ...
                      d0((1-sigma)*RR*Gx(:,pindx.lkdP))*C2P_rCutoff; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lkdP))*C2P_rCutoff + ...
                      d0(N2C*Gx(:,pindx.lkdP))*C2P_rCutoff];
		...
		if (par.opt_kdP & par.opt_PLip_scale)
			kk = kk + 1;
			dC2Ptmp = C2P_PStorscale;
			tmp = par.BIO.PStor_scale*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lkdP))*dC2Ptmp; ...
                      (1-sigma)*d0(Gx(:,pindx.lkdP))*dC2Ptmp; ...
                      sigma*d0(Gx(:,pindx.lkdP))*dC2Ptmp; ...
                      d0((1-sigma)*RR*Gx(:,pindx.lkdP))*dC2Ptmp; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lkdP))*dC2Ptmp + ...
		...
		if (par.opt_kdP & par.opt_alphaS)
			kk = kk + 1;
			dC2Ptmp = C2P_alphaS;
			tmp = par.BIO.alphaS*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lkdP))*dC2Ptmp; ...
                      (1-sigma)*d0(Gx(:,pindx.lkdP))*dC2Ptmp; ...
                      sigma*d0(Gx(:,pindx.lkdP))*dC2Ptmp; ...
                      d0((1-sigma)*RR*Gx(:,pindx.lkdP))*dC2Ptmp; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lkdP))*dC2Ptmp + ...
...
%bP_T
	...
	% bP_T + cell model
		...
		if (par.opt_bP_T & par.opt_PLip_PCutoff)
			kk = kk + 1;
			tmp = par.BIO.PLip_PCutoff*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.bP_T))*C2P_PCutoff; ...
                      (1-sigma)*d0(Gx(:,pindx.bP_T))*C2P_PCutoff; ...
                      sigma*d0(Gx(:,pindx.bP_T))*C2P_PCutoff; ...
                      d0((1-sigma)*RR*Gx(:,pindx.bP_T))*C2P_PCutoff; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.bP_T))*C2P_PCutoff + ...
                      d0(N2C*Gx(:,pindx.bP_T))*C2P_PCutoff];
		...
		if (par.opt_bP_T & par.opt_PLip_scale)
			kk = kk + 1;
			dC2Ptmp = C2P_PLipscale;
			tmp = par.BIO.PLip_scale*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.bP_T))*dC2Ptmp; ...
		...
		if (par.opt_bP_T & par.opt_PStor_rCutoff)
			kk = kk + 1;
			tmp = par.BIO.PStor_rCutoff*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.bP_T))*C2P_rCutoff; ...
                      (1-sigma)*d0(Gx(:,pindx.bP_T))*C2P_rCutoff; ...
                      sigma*d0(Gx(:,pindx.bP_T))*C2P_rCutoff; ...
                      d0((1-sigma)*RR*Gx(:,pindx.bP_T))*C2P_rCutoff; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.bP_T))*C2P_rCutoff + ...
                      d0(N2C*Gx(:,pindx.bP_T))*C2P_rCutoff];
		...
		if (par.opt_bP_T & par.opt_PStor_scale)
			kk = kk + 1;
			dC2Ptmp = C2P_PStorscale;
			tmp = par.BIO.PStor_scale*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.bP_T))*dC2Ptmp; ...
                      (1-sigma)*d0(Gx(:,pindx.bP_T))*dC2Ptmp; ...
                      sigma*d0(Gx(:,pindx.bP_T))*dC2Ptmp; ...
                      d0((1-sigma)*RR*Gx(:,pindx.bP_T))*dC2Ptmp; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.bP_T))*dC2Ptmp + ...
		...
		if (par.opt_bP_T & par.opt_alphaS)
			kk = kk + 1;
			dC2Ptmp = C2P_alphaS;
			tmp = par.BIO.alphaS*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.bP_T))*dC2Ptmp; ...
                      (1-sigma)*d0(Gx(:,pindx.bP_T))*dC2Ptmp; ...
                      sigma*d0(Gx(:,pindx.bP_T))*dC2Ptmp; ...
                      d0((1-sigma)*RR*Gx(:,pindx.bP_T))*dC2Ptmp; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.bP_T))*dC2Ptmp + ...
...
%bP
	...
	% bP + Cell model
		...
		if (par.opt_bP & par.opt_PLip_PCutoff)
			kk = kk + 1;
            tmp = par.BIO.PLip_PCutoff*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lbP))*C2P_PCutoff; ...
                      (1-sigma)*d0(Gx(:,pindx.lbP))*C2P_PCutoff;...
                      sigma*d0(Gx(:,pindx.lbP))*C2P_PCutoff;...
                      d0((1-sigma)*RR*Gx(:,pindx.lbP))*C2P_PCutoff; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lbP))*C2P_PCutoff + ...
                      d0(N2C*Gx(:,pindx.lbP))*C2P_PCutoff];
		...
		if (par.opt_bP & par.opt_PLip_scale)
			kk = kk + 1;
			dC2Ptmp = C2P_PLipscale;
            tmp = par.BIO.PLip_scale*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lbP))*dC2Ptmp; ...
		...
		if (par.opt_bP & par.opt_PStor_rCutoff)
			kk = kk + 1;
            tmp = par.BIO.PStor_rCutoff*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lbP))*C2P_rCutoff; ...
                      (1-sigma)*d0(Gx(:,pindx.lbP))*C2P_rCutoff;...
                      sigma*d0(Gx(:,pindx.lbP))*C2P_rCutoff;...
                      d0((1-sigma)*RR*Gx(:,pindx.lbP))*C2P_rCutoff; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lbP))*C2P_rCutoff + ...
                      d0(N2C*Gx(:,pindx.lbP))*C2P_rCutoff];
		...
		if (par.opt_bP & par.opt_PStor_scale)
			kk = kk + 1;
			dC2Ptmp = C2P_PStorscale;
            tmp = par.BIO.PStor_scale*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lbP))*dC2Ptmp; ...
                      (1-sigma)*d0(Gx(:,pindx.lbP))*dC2Ptmp;...
                      sigma*d0(Gx(:,pindx.lbP))*dC2Ptmp;...
                      d0((1-sigma)*RR*Gx(:,pindx.lbP))*dC2Ptmp; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lbP))*dC2Ptmp + ...
...
% alpha
	...
	% alpha +cell model
		...
		if (par.opt_alpha & par.opt_fStorage)
			kk = kk + 1;
			tmp = par.BIO.fStorage*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lalpha))*C2P_fStor; ...
		...
		if (par.opt_alpha & par.opt_PLip_PCutoff)
			kk = kk + 1;
			tmp = par.BIO.PLip_PCutoff*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lalpha))*C2P_PCutoff; ...
                      (1-sigma)*d0(Gx(:,pindx.lalpha))*C2P_PCutoff; ...
                      sigma*d0(Gx(:,pindx.lalpha))*C2P_PCutoff; ...
                      d0((1-sigma)*RR*Gx(:,pindx.lalpha))*C2P_PCutoff; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lalpha))*C2P_PCutoff + ...
                      d0(N2C*Gx(:,pindx.lalpha))*C2P_PCutoff];
		...
		if (par.opt_alpha & par.opt_PLip_scale)
			kk = kk + 1;
			dC2Ptmp = C2P_PLipscale;
			tmp = par.BIO.PLip_scale*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lalpha))*dC2Ptmp; ...
		...
		if (par.opt_alpha & par.opt_PStor_rCutoff)
			kk = kk + 1;
			tmp = par.BIO.PStor_rCutoff*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lalpha))*C2P_rCutoff; ...
                      (1-sigma)*d0(Gx(:,pindx.lalpha))*C2P_rCutoff; ...
                      sigma*d0(Gx(:,pindx.lalpha))*C2P_rCutoff; ...
                      d0((1-sigma)*RR*Gx(:,pindx.lalpha))*C2P_rCutoff; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lalpha))*C2P_rCutoff + ...
                      d0(N2C*Gx(:,pindx.lalpha))*C2P_rCutoff];
		...
		if (par.opt_alpha & par.opt_PStor_scale)
			kk = kk + 1;
			dC2Ptmp = C2P_PStorscale;
			tmp = par.BIO.PStor_scale*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lalpha))*dC2Ptmp; ...
                      (1-sigma)*d0(Gx(:,pindx.lalpha))*dC2Ptmp; ...
                      sigma*d0(Gx(:,pindx.lalpha))*dC2Ptmp; ...
                      d0((1-sigma)*RR*Gx(:,pindx.lalpha))*dC2Ptmp; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lalpha))*dC2Ptmp + ...
		...
		if (par.opt_alpha & par.opt_alphaS)
			kk = kk + 1;
			dC2Ptmp = C2P_alphaS;
			tmp = par.BIO.alphaS*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lalpha))*dC2Ptmp; ...
                      (1-sigma)*d0(Gx(:,pindx.lalpha))*dC2Ptmp; ...
                      sigma*d0(Gx(:,pindx.lalpha))*dC2Ptmp; ...
                      d0((1-sigma)*RR*Gx(:,pindx.lalpha))*dC2Ptmp; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lalpha))*dC2Ptmp + ...
...
% beta
	...
	% beta + cell model
		...
		if (par.opt_beta & par.opt_PLip_PCutoff)
			kk = kk + 1;
			tmp = par.BIO.PLip_PCutoff*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lbeta))*C2P_PCutoff; ...
                      (1-sigma)*d0(Gx(:,pindx.lbeta))*C2P_PCutoff; ...
                      sigma*d0(Gx(:,pindx.lbeta))*C2P_PCutoff; ...
                      d0((1-sigma)*RR*Gx(:,pindx.lbeta))*C2P_PCutoff; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lbeta))*C2P_PCutoff + ...
                      d0(N2C*Gx(:,pindx.lbeta))*C2P_PCutoff];
		...
		if (par.opt_beta & par.opt_PLip_scale)
			kk = kk + 1;
			dC2Ptmp = C2P_PLipscale;
			tmp = par.BIO.PLip_scale*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lbeta))*dC2Ptmp; ...
		...
		if (par.opt_beta & par.opt_PStor_rCutoff)
			kk = kk + 1;
			tmp = par.BIO.PStor_rCutoff*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lbeta))*C2P_rCutoff; ...
                      (1-sigma)*d0(Gx(:,pindx.lbeta))*C2P_rCutoff; ...
                      sigma*d0(Gx(:,pindx.lbeta))*C2P_rCutoff; ...
                      d0((1-sigma)*RR*Gx(:,pindx.lbeta))*C2P_rCutoff; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lbeta))*C2P_rCutoff + ...
                      d0(N2C*Gx(:,pindx.lbeta))*C2P_rCutoff];
		...
		if (par.opt_beta & par.opt_PStor_scale)
			kk = kk + 1;
			dC2Ptmp = C2P_PStorscale;
			tmp = par.BIO.PStor_scale*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lbeta))*dC2Ptmp; ...
                      (1-sigma)*d0(Gx(:,pindx.lbeta))*dC2Ptmp; ...
                      sigma*d0(Gx(:,pindx.lbeta))*dC2Ptmp; ...
                      d0((1-sigma)*RR*Gx(:,pindx.lbeta))*dC2Ptmp; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lbeta))*dC2Ptmp + ...
		...
		if (par.opt_beta & par.opt_alphaS)
			kk = kk + 1;
			dC2Ptmp = C2P_alphaS;
			tmp = par.BIO.alphaS*[-d0((I+(1-sigma)*RR)*Gx(:,pindx.lbeta))*dC2Ptmp; ...
                      (1-sigma)*d0(Gx(:,pindx.lbeta))*dC2Ptmp; ...
                      sigma*d0(Gx(:,pindx.lbeta))*dC2Ptmp; ...
                      d0((1-sigma)*RR*Gx(:,pindx.lbeta))*dC2Ptmp; ...
                      d0(-2*(1-sigma)*RR*Gx(:,pindx.lbeta))*dC2Ptmp + ...
	...
	%%%%% Cmodel + Cell Model %%%%%
		...
		if (par.opt_R_Si & par.opt_PLip_PCutoff)
            kk = kk + 1;
            tmp = [-par.BIO.PLip_PCutoff*(1-sigma)*RR_Si*(G*C2P_PCutoff); ...
                   Z; ...
                   Z; ...
                   par.BIO.PLip_PCutoff*(1-sigma)*RR_Si*(G*C2P_PCutoff); ...
                   -2*par.BIO.PLip_PCutoff*(1-sigma)*RR_Si*(G*C2P_PCutoff)];
		...
		if (par.opt_R_Si & par.opt_PLip_scale)
            kk = kk + 1;
			dC2Ptmp = C2P_PLipscale;
            tmp = [-par.BIO.PLip_scale*(1-sigma)*RR_Si*(G*dC2Ptmp); ...
                   Z; ...
                   Z; ...
                   par.BIO.PLip_scale*(1-sigma)*RR_Si*(G*dC2Ptmp); ...
                   -2*par.BIO.PLip_scale*(1-sigma)*RR_Si*(G*dC2Ptmp)];
		...
		if (par.opt_R_Si & par.opt_PStor_rCutoff)
            kk = kk + 1;
            tmp = [-par.BIO.PStor_rCutoff*(1-sigma)*RR_Si*(G*C2P_rCutoff); ...
                   Z; ...
                   Z; ...
                   par.BIO.PStor_rCutoff*(1-sigma)*RR_Si*(G*C2P_rCutoff); ...
                   -2*par.BIO.PStor_rCutoff*(1-sigma)*RR_Si*(G*C2P_rCutoff)];
		...
		if (par.opt_R_Si & par.opt_PStor_scale)
            kk = kk + 1;
			dC2Ptmp = C2P_PStorscale;
            tmp = [-par.BIO.PStor_scale*(1-sigma)*RR_Si*(G*dC2Ptmp); ...
                   Z; ...
                   Z; ...
                   par.BIO.PStor_scale*(1-sigma)*RR_Si*(G*dC2Ptmp); ...
                   -2*par.BIO.PStor_scale*(1-sigma)*RR_Si*(G*dC2Ptmp)];
		...
		if (par.opt_R_Si & par.opt_alphaS)
            kk = kk + 1;
			dC2Ptmp = C2P_alphaS;
            tmp = [-par.BIO.alphaS*(1-sigma)*RR_Si*(G*dC2Ptmp); ...
                   Z; ...
                   Z; ...
                   par.BIO.alphaS*(1-sigma)*RR_Si*(G*dC2Ptmp); ...
                   -2*par.BIO.alphaS*(1-sigma)*RR_Si*(G*dC2Ptmp)];
        ...
        if (par.opt_rR & par.opt_PLip_PCutoff)
            kk = kk + 1;
            tmp = [-par.BIO.PLip_PCutoff*(1-sigma)*RR_rR*(G*C2P_PCutoff); ...
                   Z; ...
                   Z; ...
                   par.BIO.PLip_PCutoff*(1-sigma)*RR_rR*(G*C2P_PCutoff); ...
                   -2*par.BIO.PLip_PCutoff*(1-sigma)*RR_rR*(G*C2P_PCutoff)];
        ...
        if (par.opt_rR & par.opt_PLip_scale)
            kk = kk + 1;
			dC2Ptmp = C2P_PLipscale;
            tmp = [-par.BIO.PLip_scale*(1-sigma)*RR_rR*(G*dC2Ptmp); ...
                   Z; ...
                   Z; ...
                   par.BIO.PLip_scale*(1-sigma)*RR_rR*(G*dC2Ptmp); ...
                   -2*par.BIO.PLip_scale*(1-sigma)*RR_rR*(G*dC2Ptmp)];
        ...
        if (par.opt_rR & par.opt_PStor_rCutoff)
            kk = kk + 1;
			dC2Ptmp = C2P_rCutoff;
            tmp = [-par.BIO.PStor_rCutoff*(1-sigma)*RR_rR*(G*dC2Ptmp); ...
                   Z; ...
                   Z; ...
                   par.BIO.PStor_rCutoff*(1-sigma)*RR_rR*(G*dC2Ptmp); ...
                   -2*par.BIO.PStor_rCutoff*(1-sigma)*RR_rR*(G*dC2Ptmp)];
        ...
        if (par.opt_rR & par.opt_PStor_scale)
            kk = kk + 1;
			dC2Ptmp = C2P_PStorscale;
            tmp = [-par.BIO.PStor_scale*(1-sigma)*RR_rR*(G*dC2Ptmp); ...
                   Z; ...
                   Z; ...
                   par.BIO.PStor_scale*(1-sigma)*RR_rR*(G*dC2Ptmp); ...
                   -2*par.BIO.PStor_scale*(1-sigma)*RR_rR*(G*dC2Ptmp)];
	...
	%%%%%%%%%%%% Cell model only %%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if (par.opt_Q10Photo & par.opt_PLip_PCutoff)
			C2P_Q10_PCutoff = par.CellOut.d2C2P_dPCutoff_dQ10Photo(iwet);
            kk = kk + 1;
            tmp = par.BIO.Q10Photo*par.BIO.PLip_PCutoff*[-(I+(1-sigma)*RR)*(G*C2P_Q10_PCutoff); ...
                         (1-sigma)*G*C2P_Q10_PCutoff; ...
                         sigma*G*C2P_Q10_PCutoff; ...
                         (1-sigma)*RR*(G*C2P_Q10_PCutoff); ...
                         -2*(1-sigma)*RR*(G*C2P_Q10_PCutoff) + ...
                         N2C*G*C2P_Q10_PCutoff];

            Cxx(:,kk) = mfactor(FD, tmp);
        end
		% Q10Photo PLip_scale
        if (par.opt_Q10Photo & par.opt_PLip_scale)
			%C2P_Q10_PLipscale = par.CellOut.d2C2P_dPLipscale_dQ10Photo(iwet); dC2Ptmp=C2P_Q10_PLipscale;
			dC2Ptmp = par.CellOut.d2C2P_dPLipscale_dQ10Photo(iwet);
            kk = kk + 1;
            tmp = par.BIO.Q10Photo*par.BIO.PLip_scale*[-(I+(1-sigma)*RR)*(G*dC2Ptmp); ...
                         (1-sigma)*G*dC2Ptmp; ...
                         sigma*G*dC2Ptmp; ...
                         (1-sigma)*RR*(G*dC2Ptmp); ...
                         -2*(1-sigma)*RR*(G*dC2Ptmp) + ...
        ...
        if (par.opt_Q10Photo & par.opt_PStor_rCutoff)
			C2P_Q10_rCutoff = par.CellOut.d2C2P_drCutoff_dQ10Photo(iwet);
            kk = kk + 1;
            tmp = par.BIO.Q10Photo*par.BIO.PStor_rCutoff*[-(I+(1-sigma)*RR)*(G*C2P_Q10_rCutoff); ...
                         (1-sigma)*G*C2P_Q10_rCutoff; ...
                         sigma*G*C2P_Q10_rCutoff; ...
                         (1-sigma)*RR*(G*C2P_Q10_rCutoff); ...
                         -2*(1-sigma)*RR*(G*C2P_Q10_rCutoff) + ...
                         N2C*G*C2P_Q10_rCutoff];

            Cxx(:,kk) = mfactor(FD, tmp);
        end
		% Q10Photo PStor_scale
        if (par.opt_Q10Photo & par.opt_PStor_scale)
			dC2Ptmp = par.CellOut.d2C2P_dPStorscale_dQ10Photo(iwet);
            kk = kk + 1;
            tmp = par.BIO.Q10Photo*par.BIO.PLip_scale*[-(I+(1-sigma)*RR)*(G*dC2Ptmp); ...
                         (1-sigma)*G*dC2Ptmp; ...
                         sigma*G*dC2Ptmp; ...
        ...
        if (par.opt_Q10Photo & par.opt_alphaS)
			dC2Ptmp = par.CellOut.d2C2P_dalphaS_dQ10Photo(iwet);
            kk = kk + 1;
            tmp = par.BIO.Q10Photo*par.BIO.PLip_scale*[-(I+(1-sigma)*RR)*(G*dC2Ptmp); ...
                         (1-sigma)*G*dC2Ptmp; ...
                         sigma*G*dC2Ptmp; ...
        ...
        if (par.opt_fStorage & par.opt_PLip_PCutoff)
			C2P_fStor_PCutoff = par.CellOut.d2C2P_dPCutoff_dfStorage(iwet);
            kk = kk + 1;
            tmp = par.BIO.fStorage*par.BIO.PLip_PCutoff*[-(I+(1-sigma)*RR)*(G*C2P_fStor_PCutoff); ...
                         (1-sigma)*G*C2P_fStor_PCutoff; ...
                         sigma*G*C2P_fStor_PCutoff; ...
                         (1-sigma)*RR*(G*C2P_fStor_PCutoff); ...
                         -2*(1-sigma)*RR*(G*C2P_fStor_PCutoff) + ...
                         N2C*G*C2P_fStor_PCutoff];

            Cxx(:,kk) = mfactor(FD, tmp);
        end
		% fStorage PLip_scale
        if (par.opt_fStorage & par.opt_PLip_scale)
			dC2Ptmp = par.CellOut.d2C2P_dPLipscale_dfStorage(iwet);
            kk = kk + 1;
            tmp = par.BIO.fStorage*par.BIO.PLip_scale*[-(I+(1-sigma)*RR)*(G*dC2Ptmp); ...
                         (1-sigma)*G*dC2Ptmp; ...
                         sigma*G*dC2Ptmp; ...
                         (1-sigma)*RR*(G*dC2Ptmp); ...
                         -2*(1-sigma)*RR*(G*dC2Ptmp) + ...
        ...
        if (par.opt_fStorage & par.opt_PStor_rCutoff)
			C2P_fStor_rCutoff = par.CellOut.d2C2P_drCutoff_dfStorage(iwet);
            kk = kk + 1;
            tmp = par.BIO.fStorage*par.BIO.PStor_rCutoff*[-(I+(1-sigma)*RR)*(G*C2P_fStor_rCutoff); ...
                         (1-sigma)*G*C2P_fStor_rCutoff; ...
                         sigma*G*C2P_fStor_rCutoff; ...
                         (1-sigma)*RR*(G*C2P_fStor_rCutoff); ...
                         -2*(1-sigma)*RR*(G*C2P_fStor_rCutoff) + ...
                         N2C*G*C2P_fStor_rCutoff];

            Cxx(:,kk) = mfactor(FD, tmp);
        end
		% fStorage PStor_scale
        if (par.opt_fStorage & par.opt_PStor_scale)
			dC2Ptmp = par.CellOut.d2C2P_dPStorscale_dfStorage(iwet);
            kk = kk + 1;
            tmp = par.BIO.fStorage*par.BIO.PStor_scale*[-(I+(1-sigma)*RR)*(G*dC2Ptmp); ...
                         (1-sigma)*G*dC2Ptmp; ...
                         sigma*G*dC2Ptmp; ...
                         (1-sigma)*RR*(G*dC2Ptmp); ...
                         -2*(1-sigma)*RR*(G*dC2Ptmp) + ...
        ...
        if (par.opt_fStorage & par.opt_alphaS)
			dC2Ptmp = par.CellOut.d2C2P_dalphaS_dfStorage(iwet);
            kk = kk + 1;
            tmp = par.BIO.fStorage*par.BIO.alphaS*[-(I+(1-sigma)*RR)*(G*dC2Ptmp); ...
                         (1-sigma)*G*dC2Ptmp; ...
                         sigma*G*dC2Ptmp; ...
                         (1-sigma)*RR*(G*dC2Ptmp); ...
                         -2*(1-sigma)*RR*(G*dC2Ptmp) + ...
		...
		if (par.opt_PLip_PCutoff)
			C2P_PCutoff_PCutoff = par.CellOut.d2C2P_dPCutoff2(iwet);
            kk = kk + 1;
            tmp = par.BIO.PLip_PCutoff*[-(I+(1-sigma)*RR)*(G*(C2P_PCutoff+par.BIO.PLip_PCutoff*C2P_PCutoff_PCutoff)); ...
                      (1-sigma)*G*(C2P_PCutoff+par.BIO.PLip_PCutoff*C2P_PCutoff_PCutoff); ...
                      sigma*G*(C2P_PCutoff+par.BIO.PLip_PCutoff*C2P_PCutoff_PCutoff); ...
                      (1-sigma)*RR*(G*(C2P_PCutoff+par.BIO.PLip_PCutoff*C2P_PCutoff_PCutoff)); ...
                      -2*(1-sigma)*RR*(G*(C2P_PCutoff+par.BIO.PLip_PCutoff*C2P_PCutoff_PCutoff)) + ...
                      N2C*G*(C2P_PCutoff+par.BIO.PLip_PCutoff*C2P_PCutoff_PCutoff)];

            Cxx(:,kk) = mfactor(FD, tmp);
        end
		% PLip_PCutoff PLip_scale
        if (par.opt_PLip_PCutoff & par.opt_PLip_scale)
			%C2P_PCutoff_PLipscale = par.CellOut.d2C2P_dPLipscale_dPCutoff(iwet);
            dC2Ptmp=par.CellOut.d2C2P_dPLipscale_dPCutoff(iwet);
            kk = kk + 1;
            tmp = par.BIO.PLip_PCutoff*par.BIO.PLip_scale*[-(I+(1-sigma)*RR)*(G*dC2Ptmp); ...
                         (1-sigma)*G*dC2Ptmp; ...
                         sigma*G*dC2Ptmp; ...
                         (1-sigma)*RR*(G*dC2Ptmp); ...
                         -2*(1-sigma)*RR*(G*dC2Ptmp) + ...
        ...
        if (par.opt_PLip_PCutoff & par.opt_PStor_rCutoff)
            ...
            dC2Ptmp=C2P_PCutoff_rCutoff;
            kk = kk + 1;
            tmp = par.BIO.PLip_PCutoff*par.BIO.PStor_rCutoff*[-(I+(1-sigma)*RR)*(G*dC2Ptmp); ...
                         (1-sigma)*G*dC2Ptmp; ...
                         sigma*G*dC2Ptmp; ...
                         (1-sigma)*RR*(G*dC2Ptmp); ...
                         -2*(1-sigma)*RR*(G*dC2Ptmp) + ...
        ...
        if (par.opt_PLip_PCutoff & par.opt_PStor_scale)
            dC2Ptmp=par.CellOut.d2C2P_dPStorscale_dPCutoff(iwet);
            kk = kk + 1;
            tmp = par.BIO.PLip_PCutoff*par.BIO.PStor_scale*[-(I+(1-sigma)*RR)*(G*dC2Ptmp); ...
                         (1-sigma)*G*dC2Ptmp; ...
                         sigma*G*dC2Ptmp; ...
                         (1-sigma)*RR*(G*dC2Ptmp); ...
                         -2*(1-sigma)*RR*(G*dC2Ptmp) + ...
        ...
        if (par.opt_PLip_PCutoff & par.opt_alphaS)
            dC2Ptmp=par.CellOut.d2C2P_dalphaS_dPCutoff(iwet);
            kk = kk + 1;
            tmp = par.BIO.PLip_PCutoff*par.BIO.alphaS*[-(I+(1-sigma)*RR)*(G*dC2Ptmp); ...
                         (1-sigma)*G*dC2Ptmp; ...
                         sigma*G*dC2Ptmp; ...
                         (1-sigma)*RR*(G*dC2Ptmp); ...
                         -2*(1-sigma)*RR*(G*dC2Ptmp) + ...
		...
		if (par.opt_PLip_scale)
			C2P_PLipscale_PLipscale = par.CellOut.d2C2P_dPLipscale2(iwet);
            dC2Ptmp = C2P_PLipscale+par.BIO.PLip_scale*C2P_PLipscale_PLipscale;
            kk = kk + 1;
            tmp = par.BIO.PLip_scale*[-(I+(1-sigma)*RR)*(G*(dC2Ptmp)); ...
                      (1-sigma)*G*(dC2Ptmp); ...
                      sigma*G*(dC2Ptmp); ...
                      (1-sigma)*RR*(G*(dC2Ptmp)); ...
                      -2*(1-sigma)*RR*(G*(dC2Ptmp)) + ...
        ...
        if (par.opt_PLip_scale & par.opt_PStor_rCutoff)
            dC2Ptmp = par.CellOut.d2C2P_drCutoff_dPLipscale(iwet);
            kk = kk + 1;
            tmp = par.BIO.PLip_scale*par.BIO.PStor_rCutoff*[-(I+(1-sigma)*RR)*(G*dC2Ptmp); ...
                         (1-sigma)*G*dC2Ptmp; ...
                         sigma*G*dC2Ptmp; ...
                         (1-sigma)*RR*(G*dC2Ptmp); ...
                         -2*(1-sigma)*RR*(G*dC2Ptmp) + ...
        ...
        if (par.opt_PLip_scale & par.opt_PStor_scale)
            dC2Ptmp = par.CellOut.d2C2P_dPStorscale_dPLipscale(iwet);
            kk = kk + 1;
            tmp = par.BIO.PLip_scale*par.BIO.PStor_scale*[-(I+(1-sigma)*RR)*(G*dC2Ptmp); ...
                         (1-sigma)*G*dC2Ptmp; ...
                         sigma*G*dC2Ptmp; ...
                         (1-sigma)*RR*(G*dC2Ptmp); ...
                         -2*(1-sigma)*RR*(G*dC2Ptmp) + ...
        ...
        if (par.opt_PLip_scale & par.opt_alphaS)
            dC2Ptmp = par.CellOut.d2C2P_alphaS_dPLipscale(iwet);
            kk = kk + 1;
            tmp = par.BIO.PLip_scale*par.BIO.alphaS*[-(I+(1-sigma)*RR)*(G*dC2Ptmp); ...
                         (1-sigma)*G*dC2Ptmp; ...
                         sigma*G*dC2Ptmp; ...
                         (1-sigma)*RR*(G*dC2Ptmp); ...
                         -2*(1-sigma)*RR*(G*dC2Ptmp) + ...
		...
		if (par.opt_PStor_rCutoff)
			C2P_rCutoff_rCutoff = par.CellOut.d2C2P_drCutoff2(iwet);
            dC2Ptmp = C2P_rCutoff+par.BIO.PStor_rCutoff*C2P_rCutoff_rCutoff;
            kk = kk + 1;
            tmp = par.BIO.PStor_rCutoff*[-(I+(1-sigma)*RR)*(G*(dC2Ptmp)); ...
                      (1-sigma)*G*(dC2Ptmp); ...
                      sigma*G*(dC2Ptmp); ...
                      (1-sigma)*RR*(G*(dC2Ptmp)); ...
                      -2*(1-sigma)*RR*(G*(dC2Ptmp)) + ...
		...
		if (par.opt_PStor_rCutoff & par.opt_PStor_scale)
            dC2Ptmp=par.CellOut.d2C2P_dPStorscale_drCutoff(iwet);
            kk = kk + 1;
            tmp = par.BIO.PStor_rCutoff*par.BIO.PStor_scale*[-(I+(1-sigma)*RR)*(G*dC2Ptmp); ...
                         (1-sigma)*G*dC2Ptmp; ...
                         sigma*G*dC2Ptmp; ...
                         (1-sigma)*RR*(G*dC2Ptmp); ...
                         -2*(1-sigma)*RR*(G*dC2Ptmp) + ...
		...
		if (par.opt_PStor_rCutoff & par.opt_alphaS)
            dC2Ptmp=par.CellOut.d2C2P_dalphaS_drCutoff(iwet);
            kk = kk + 1;
            tmp = par.BIO.PStor_rCutoff*par.BIO.alphaS*[-(I+(1-sigma)*RR)*(G*dC2Ptmp); ...
                         (1-sigma)*G*dC2Ptmp; ...
                         sigma*G*dC2Ptmp; ...
                         (1-sigma)*RR*(G*dC2Ptmp); ...
                         -2*(1-sigma)*RR*(G*dC2Ptmp) + ...
		...
		if (par.opt_PStor_scale)
			C2P_PStorscale_PStorscale = par.CellOut.d2C2P_dPStorscale2(iwet);
            dC2Ptmp = C2P_PStorscale+par.BIO.PLip_scale*C2P_PStorscale_PStorscale;
            kk = kk + 1;
            tmp = par.BIO.PStor_scale*[-(I+(1-sigma)*RR)*(G*(dC2Ptmp)); ...
                      (1-sigma)*G*(dC2Ptmp); ...
                      sigma*G*(dC2Ptmp); ...
                      (1-sigma)*RR*(G*(dC2Ptmp)); ...
                      -2*(1-sigma)*RR*(G*(dC2Ptmp)) + ...
		...
		if (par.opt_PStor_scale & par.opt_alphaS)
            dC2Ptmp=par.CellOut.d2C2P_dalphaS_dPStorscale(iwet);
            kk = kk + 1;
            tmp = par.BIO.PStor_scale*par.BIO.alphaS*[-(I+(1-sigma)*RR)*(G*dC2Ptmp); ...
                         (1-sigma)*G*dC2Ptmp; ...
                         sigma*G*dC2Ptmp; ...
                         (1-sigma)*RR*(G*dC2Ptmp); ...
                         -2*(1-sigma)*RR*(G*dC2Ptmp) + ...
		...
		% alphaS alphaS
		if (par.opt_alphaS)
			C2P_alphaS_alphaS = par.CellOut.d2C2P_dalphaS2(iwet);
            dC2Ptmp = C2P_alphaS+par.BIO.alphaS*C2P_alphaS_alphaS;
            kk = kk + 1;
            tmp = par.BIO.alphaS*[-(I+(1-sigma)*RR)*(G*(dC2Ptmp)); ...
                      (1-sigma)*G*(dC2Ptmp); ...
                      sigma*G*(dC2Ptmp); ...
                      (1-sigma)*RR*(G*(dC2Ptmp)); ...
                      -2*(1-sigma)*RR*(G*(dC2Ptmp)) + ...
